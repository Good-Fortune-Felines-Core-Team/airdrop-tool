...
(async () => {
    ...
    const signerPubKey = await account.connection.signer.getPublicKey(accountId, "mainnet");
    ...
    async function runMain() {
        ...
        const listFile = await fs.readFile(join(__dirname, "..", "data", listName + ".json"), "utf-8");
        const list = JSON.parse(listFile);
        let promiseArray = [];
        const accessKey = await account.connection.provider.query(`access_key/${accountId}/${signerPubKey.toString()}`, "");
            ...
            const promise = new Promise(async (resolve) => {
                ...
                try {
                    ...
                    const accountData = await nearConnection.connection.provider
                        ...
                        await fs.appendFile(join(__dirname, "..", "error", listName + ".txt"), `${id}:${transferAmount.toString()}\r\n`);
                        delete list[id];
                        await fs.writeFile(join(__dirname, "..", "data", listName + ".json"), JSON.stringify(list));
                    });
                    if (!accountData) {
                        resolve();
                        return;
                    ...
                    const signedTransaction = await transactions.signTransaction(transaction, account.connection.signer, accountId, "mainnet");
        ...
        if (Object.keys(list).length === 0) {
            await fs.rename(join(__dirname, "..", "data", listName + ".json"), join(__dirname, "..", "finished", listName + ".json"));
